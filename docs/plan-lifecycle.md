...
7. If `codereview` is `ask` or `always`, RunWield opens or offers Plannotator Code Review after Semantic Review passes
   ...
   Review then reopens. Code Review always sees the full workflow diff, never a repair-scoped one. User approval reached
   through the round-limit escape hatch is authoritative and permits merge-back even though Semantic Review never
   approved; the record distinguishes that case.
